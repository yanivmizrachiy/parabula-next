...
const TITLES = [
  'זווית ישרה','משולש ישר־זווית','ניצבים','היתר','זיהוי ניצבים ויתר','חזקה שנייה','ריבועי מספרים','שורש ריבועי',
  'מציאת אורך מתוך x² = n','שורשים שאינם שלמים','מציאת אורך כאשר השורש אינו שלם','ריבוע על צלע','הקשר בין ריבועי הצלעות',
  'משפט פיתגורס','נוסחת משפט פיתגורס','כתיבת משוואת פיתגורס','דרך פתרון באמצעות משפט פיתגורס','מציאת היתר באמצעות משפט פיתגורס',
  'מציאת ניצב באמצעות משפט פיתגורס','בחירת דרך הפתרון במשפט פיתגורס'
];
...
test('20 דפי היסוד שומרים בדיוק על סדר המושגים המחייב', () => {
  const issues = [];
...
test('הוראה חדשה נשענת על מקבצים ושומרת משימות מועילות קיימות', () => {
  ...
  const p4 = htmlOf(637);
  assert.ok((p4.match(/aria-label="כתבו את אורך היתר"/gu)||[]).length >= 3,'עמוד 4: זיהוי היתר לפי אורכים חייב להיות מקבץ');
});
