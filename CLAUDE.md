...
נכון ל-2026-07-11 (אחרי ייבוא bbb ספרים 1-3):

- 230 קובצי דפי עבודה קנוניים בשורש: `עמוד-N.html`.
- 230 קובצי CSS ייעודיים תחת `styles/pages/`.
- 11 נושאים ב-`meta/topics.json`.
...
הספר הרביעי — "תחום אי־וודאות" (55 שאלות) — טרם יובא: 3 שאלות בעלות פתיח figure/chart גבוה
לפני רשימת הסעיפים דורשות פיצול-פתיח רקורסיבי נאמן-לטקסט שעדיין לא יציב (ראה §17).

אם דוח ישן מציין 98/95 דפים או 8/7 נושאים, הוא מיושן.
...
11. **ייבוא bbb — הספר הרביעי ("תחום אי־וודאות", 55 שאלות) טרם יובא.** 3 שאלות (Q12/Q14/Q53 ב-`sources/bbb/uncertainty/worksheet.html`) בעלות פתיח figure+chart גבוה מדף לפני רשימת הסעיפים. `scripts/import-bbb.mjs` מפצל שאלות גבוהות בגבול תת-סעיף/בלוק/שורת-טבלה, אבל פיצול-הפתיח הרקורסיבי דרך `splitQbodyBlocks` עדיין שובר את בדיקת הנאמנות (שחזור ה-lead/tail של qbody לא מדויק ב-100%). הפתרון: לחזק את `splitQbodyBlocks` לשמירת סדר inline/טקסט מדויק, או להוסיף מנגנון פיצול-פתיח ייעודי. הספר יובא ברגע שהפיצול יציב. ספרים 1-3 (132 דפים מיובאים) חיים ומאומתים מלא.
