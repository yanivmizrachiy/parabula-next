...
8. PR #28 ממתין למיזוג של יניב: המרת 52 דפי `משוואות` ל-HTML+MathJax חי (פותר את בעיה 2) + יישור ולידטורי המשוואות. אחרי המיזוג יש להריץ `node scripts/vendorize-cdn.mjs` (עצמאות מ-CDN) ולקפל את `WORKSHEET_AUTHORING_GUIDE.md` לתוך קובץ זה.
9. שתי בדיקות ישנות ב-`tests/preview.rules.test.mjs` (topic buttons UI, fitA4InHost) מצפות לעיצוב Reader ישן ונכשלות; אינן ב-CI. יש ליישב מול העיצוב הנוכחי של ה-reader.
