...
13. **המרת raster→vector (בתהליך, החל 2026-07-12).** דפי ייבוא bbb + עמוד 93/94 מכילים `<img>` (PNG/תמונת-עמוד). **סיווג מחייב:** תרשימים מתמטיים (גרפי-קואורדינטות, עוגות, צורות עם מידות, טבלאות) → **מומרים ל-SVG/HTML חי** נאמן (מדידה מהמקור, בלי ניחוש §1.2). **צילומים ואיורים דקורטיביים** שהשאלה מפנה אליהם ("בתמונה" — קרש-גיהוץ, פיקניק, צנצנות, אופניים) → **נשארים raster** (המרה = זיוף, §8 vector הוא לגרפיקה מתמטית בלבד). תשתית: מחולל `coord-svg` + CSS משותף `cg-*` ב-`bbb-algebra8.css` (רשת/צירים/נקודות אחידים, `direction:ltr` למספרים שליליים). המחולל תומך: ישרים · 4-רביעים · נקודות · piecewise · סכמטי (ללא סקאלה) · צירי-זמן · עקומות דעיכה · 2-קווים+מקרא · עיגול-פתוח. **הושלם ונפרס חי (17 דפים):** 237 (עוגה — מדידה פיקסלית: היסטוריה=30), 152, 155, 153, 156, 191, 162, 139, 141, 145, 148, 143, 137, 144, 159, 161, 175. כל דף אומת דפדפן+נייד (8 shards) + 0 overflow + contracts 268/0. **נכסי raster מתים נמחקו תוך-כדי** (63 קבצים: PNG של דפים שהומרו + 48 SVG-משוואות ישנים מ-PR#28; page-53/54 נשמרו לעמוד-93/94). **נשארים raster במכוון** (vector לא נאמן/לא מתאים): צילומים ואיורים (198 אופניים, 208 צנצנות, 212 גיהוץ, 213 פיקניק), נתוני-אמת צפופים (135 מליחות הכנרת ~60 נקודות), מורכב-אמביגואי (231 ארבעה-תרשימים, 194). **נותרו:** ~9 גרפי algebra8 + 4 תרשימי גאומטריה (גלגל-ענק/טבעת/גלילים) + 93/94 (→MathJax). עבודת אצוות רב-סבבית; כל אצווה ממוזגת ל-main ונפרסת חי.
