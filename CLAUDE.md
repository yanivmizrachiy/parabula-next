...
נכון ל-2026-07-11 (אחרי ייבוא bbb — כל 4 הספרים):

- 267 קובצי דפי עבודה קנוניים בשורש: `עמוד-N.html`.
- 267 קובצי CSS ייעודיים תחת `styles/pages/`.
- 12 נושאים ב-`meta/topics.json`.
...
9. אלגברה לכיתה ז' — 26 דפים (עמוד-99..124, ייבוא bbb).
10. אלגברה לכיתה ח' — 71 דפים (עמוד-125..195, ייבוא bbb).
11. גאומטריה לכיתה ח' — 35 דפים (עמוד-196..230, ייבוא bbb).
12. תחום אי־וודאות — 37 דפים (עמוד-231..267, ייבוא bbb).

ייבוא bbb (ריפו קריאה-בלבד https://github.com/yanivmizrachiy/bbb) מתבצע ע"י `scripts/import-bbb.mjs`
בתמלול נאמן 100% (כל שאלה זהה למקור, MathJax מדויק). המקור נשמר תחת `sources/bbb/`.
**כל 4 הספרים יובאו** (263 שאלות → 169 דפים). המנוע מפצל שאלות גבוהות מ-A4 בגבול
תת-סעיף/בלוק/שורת-טבלה/עמודה בנאמנות מלאה, עם מדידת דפדפן דטרמיניסטית.

אם דוח ישן מציין 230/98/95 דפים או 11/8/7 נושאים, הוא מיושן.
...
- אין ולא יהיה קובץ ראי נוסף למטא-דאטה — הנייח והנייד קוראים ישירות מ-`meta/topics.json` בלבד; הבדיקות (`validate:access`, `validate:mobile`, `rules:check`) נכשלות אם ראי כזה חוזר.
...
- `vendor/` מכיל עותקים מקומיים של MathJax **4.1.3** (הבאנדל `tex-mml-chtml.js` עם גופן `mathjax-tex` מובנה + `tex-font/chtml/woff2` — 22 קבצי woff2 + `sre/` — worker הנגישות ו-mathmaps) ו-Rubik, לצורך עצמאות מ-CDN והדפסה ללא אינטרנט; שני מסלולי הפריסה מעתיקים אותו ל-`dist/`. **קריטי:** MathJax 4 טוען את `sre/speech-worker.js` באתחול ללא-תנאי (לא ניתן לכבות ב-config); בלי `vendor/mathjax/sre/` נזרקת שגיאת 404 ו-`MathJax.startup.promise` אינו נפתר — מה שתוקע כל בדיקה שממתינה לו. חובה לשמור את `sre/` תחת `vendor/mathjax/`.
...
7. מריצים `npm test`, `npm run verify`, `npm run validate:access` ו-`npm run validate:mobile`, ולפי היקף `npm run tech:max`.
...
10. ~~דפי `משפט פיתגורס` 13–30 הם דפי תמונת-SVG של מקור PDF~~ — **נפתר 2026-07-12 באישור יניב.** כל 18 הדפים (עמוד-13..30, מקור `sources/bbb/`-חיצוני `משפט פיתגורס לכיתה ח.pdf` עמ' 5–22) הומרו ל-HTML+MathJax+SVG חי ונאמן, בעיצוב `styles/topics/pythagoras-live.css` (scoped `.pyt-live`). תמלול מדויק, סרטוט וקטורי מחושב מפיתגורס (§8), ניצול A4 100%. תשובות מוצגות רק בדפים שהמקור מציג בהם מפתח (13–16, 24–29). אומת ב-3 מעברי אימות אדוורסריים + deploy עם סריקת נייד מלאה. שתי טעויות שהיו במקור **תוקנו לערך הנכון באישור יניב** (2026-07-12): עמוד-15 סרטוט ב' → \(x=CE=26\) (במקום 8.94 השגוי); עמוד-14 → היקף \(60\) מ' (במקום "ס״מ" השגוי, כי התרגיל כולו במטרים).
...
12. ~~שדרוג מנוע הנוסחאות~~ — **בוצע, מוזג ל-`main` ונפרס לאתר החי 2026-07-12.** MathJax שודרג 3.2.2 → **4.1.3** self-hosted, עם גופן `mathjax-tex` (זהות ויזואלית מלאה ל-v3) — כדי לקבל את שיפורי מנוע v4 (יישור baseline ב-Safari/iOS, `blacker` strokes להדפסה, מפרידים נמתחים) **בלי לשנות את מראה הנוסחאות**. הבאנדל `vendor/mathjax/tex-mml-chtml.js` (v4, גופן מובנה) + `vendor/mathjax/tex-font/chtml/woff2` (22 woff2, 348KB) החליפו את גופני v3. **בנוסף וונדר `vendor/mathjax/sre/`** (worker הנגישות + mathmaps, 4.6MB): MathJax 4 טוען את ה-worker באתחול ללא-תנאי, ובלעדיו נזרקת שגיאת 404 ו-`startup.promise` אינו נפתר (זוהה כשהסריקה נתקעה — כל בדיקה שממתינה ל-`startup.promise` תלתה; אחרי הוונדור `startup.promise` נפתר ב-~267ms). **אימות סופי:** `npm run validate:mobile:all-pages` עבר **801/801 בדיקות, 0 כשלים** (כל 267 הדפים × 3 viewports, בקורא וב-`פתח מלא`) על MathJax 4.1.3. כל 267 הדפים + התבניות (`new-page.mjs`, `import-bbb.mjs`, `vendorize-cdn.mjs`) + §5/§4.7 עודכנו ל-`fontURL: 'vendor/mathjax/tex-font/chtml/woff2'`. אומת: `ci:all` ירוק (חוזים 268/0, build 267 דפים, vendorize:check OK) + אימות דפדפן חי על 6 דפים מכל הנושאים (MathJax 4.1.3, 0 שגיאות, 0 overflow ב-A4, 0 גלילה אופקית) + סריקת נייד גאומטרית מלאה על כל הדפים. הבחירה בגופן `mathjax-tex` (ולא `mathjax-newcm` ברירת המחדל של v4) מכוונת: שומרת מטריקות זהות ל-v3 ומונעת שינויי פריסה על פני 267 דפים. **פריסה:** מוזג ל-`main` (merge commit), workflow `deploy-pages` עבר ירוק כולל **כל 8 shards של סריקת הנייד** (~2:30 כל אחד) + build + deploy; האתר החי אומת בדפדפן (`עמוד-31.html`: MathJax 4.1.3, 37 נוסחאות, 0 שגיאות console, 0 overflow, גופני v4 נטענים 200).
11. ~~ייבוא bbb הספר הרביעי~~ — **הושלם 2026-07-11.** כל 4 הספרים יובאו (169 דפים). התיקונים המרכזיים שאיפשרו זאת: (א) מדידת דפדפן דטרמיניסטית (מדידה חוזרת עד התייצבות — מנעה מרוץ שבו טבלה נמדדת קצר/גבוה); (ב) `splitQbodyBlocks` מבוסס יחידות-רציפות (שרשור = qbody מקורי → נאמנות מובטחת) עם זיהוי בלוקים ע"י קפיצה מעל כל בלוק (עמיד ל-SVG לא-מאוזן); (ג) פיצול twocol/table(שורות tbody, thead בעמוד הראשון)/lines; (ד) CSS ל-`.iconrow` (אייקוני-תרשימים קטנים בשורה) — חייב לשבת **לפני** בלוק `BBB_GENERATED_STYLES` ב-CSS הנושא, אחרת `writeGeneratedCss` מוחק אותו.
13. **המרת raster→vector (בתהליך, החל 2026-07-12).** דפי ייבוא bbb + עמוד 93/94 מכילים `<img>` (PNG/תמונת-עמוד). **סיווג מחייב:** תרשימים מתמטיים (גרפי-קואורדינטות, עוגות, צורות עם מידות, טבלאות) → **מומרים ל-SVG/HTML חי** נאמן (מדידה מהמקור, בלי ניחוש §1.2). **צילומים ואיורים דקורטיביים** שהשאלה מפנה אליהם ("בתמונה" — קרש-גיהוץ, פיקניק, צנצנות, אופניים) → **נשארים raster** (המרה = זיוף, §8 vector הוא לגרפיקה מתמטית בלבד). תשתית: מחולל `coord-svg` + CSS משותף `cg-*` ב-`bbb-algebra8.css` (רשת/צירים/נקודות אחידים, `direction:ltr` למספרים שליליים). המחולל תומך: ישרים · 4-רביעים · נקודות · piecewise · סכמטי (ללא סקאלה) · צירי-זמן · עקומות דעיכה · 2-קווים+מקרא · עיגול-פתוח. המחולל תומך גם: גלגל גאומטרי (`wheel-svg`, תאים שווי-מרחק) · setup-block (טקסט-בעיה חי). **הושלם ונפרס חי (21 דפים) — שלב התרשימים הנקיים גמור:** 237 (עוגה — מדידה פיקסלית: היסטוריה=30), 152, 155, 153, 156, 191, 162, 139, 141, 145, 148, 143, 137, 144, 159, 161, 175, 131 (גלגל-ענק), 173, 171 (משולשים), 174 (טקסט→HTML). כל דף אומת דפדפן+נייד (8 shards) + 0 overflow + contracts 268/0. **67 נכסי raster מתים נמחקו תוך-כדי** (PNG של דפים שהומרו + 48 SVG-משוואות ישנים מ-PR#28; page-53/54 נשמרו לעמוד-93/94). **נשארים raster במכוון — נבדקו כולם, המרה = זיוף/איכות נמוכה/סיכון:** צילומים+איורים+קומפוזיט (198,204,208,212,213,150,203), 3D-פרספקטיבה (206,207), נתוני-אמת צפופים ~60 נקודות (135,128), קומפוזיט רב-פאנל/מסיחים (231,132,142,163,177,178,180,185,186,195,176,158), אמביגואי-נקודות-לא-עקביות (136,194). **הושלם — עמוד-93/94 (§12):** דפי המשוואות 53/54 (תמונת-SVG) הומרו ל-HTML+MathJax חי בתמלול נאמן (6 משוואות רציונליות כל אחד + אזורי פתרון graph-paper); page-53/54.svg נמחקו. **הפרויקט הושלם: 23 דפים הומרו לחי, 25 נשארים raster מוצדק (צילומים/3D/נתוני-אמת/קומפוזיט/אמביגואי). אפס `<img>` raster שהיה צריך להיות vector.** כל אצווה מוזגה ל-main ונפרסה חי; 69 נכסי raster מתים נמחקו תוך-כדי.
