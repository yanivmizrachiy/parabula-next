...
for (let i = 0; i < topic.pages.length; i += 1) {
  ...
  const finalAnswers = count(html, /class="[^"]*(?:student-final-answer|pyt-final-answer|problem-answer|final-answer|answer-box)[^"]*"/gu) + count(html, /aria-label="אפשרויות לתשובה הסופית"/gu);
  ...
  rows.push({ local: i + 1, file: page.file, workAreas: measuredAreas, declared: explicitLineCounts.length, minDeclared, finalAnswers, fullCards, calcSignals });
...
for (const r of rows) {
  if (r.workAreas || r.calcSignals) console.log(`${String(r.local).padStart(2)} | ${r.file} | ${r.workAreas} | ${r.declared} | ${r.minDeclared ?? '-'} | ${r.finalAnswers} | ${r.fullCards} | ${r.calcSignals}`);
}
...
if (issues.length) {
  ...
  console.log('\n[OK] המטא של אזורי הכתיבה תקין; שיוך סעיפים, תשובות וגובה פיזי נאכפים בדפדפן.');
}
