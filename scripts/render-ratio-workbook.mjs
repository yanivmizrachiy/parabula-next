...
try {
  ...
  for (let pageNumber = 1; pageNumber <= pageCount; pageNumber += 1) {
    ...
    if (verticalOverflow > 1 || horizontalOverflow > 1) {
      throw new Error(`Page ${pageNumber} overflows: vertical=${verticalOverflow}px horizontal=${horizontalOverflow}px`);
    }
    if (Math.abs(geometry.width - 793.7) > 2 || Math.abs(geometry.height - 1122.5) > 2) {
      throw new Error(`Page ${pageNumber} is not A4: ${geometry.width}×${geometry.height}`);
    }
    if (consoleErrors.length > 0) {
      throw new Error(`Page ${pageNumber} emitted browser errors: ${consoleErrors.join(' | ')}`);
    }
    ...
    process.stdout.write(`Rendered ratio page ${pageNumber}/${pageCount}\n`);
  }

  await fs.writeFile(
    path.join(outputDir, 'audit-v2.json'),
    `${JSON.stringify({ generatedAt: new Date().toISOString(), pages: audit }, null, 2)}\n`,
    'utf8',
  );
  await context.close();
} finally {
