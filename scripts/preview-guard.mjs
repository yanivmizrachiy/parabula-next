...
const targets = ['preview/app.html','preview/phone.html','preview/install.html','preview/print.html'];
...
for (const rel of targets) {
  ...
  if (!row.exists) {
    ...
    row.has_inline_style = txt.includes('<style');
    if (!row.has_ux_css) report.summary.missing_ux_css += 1;
