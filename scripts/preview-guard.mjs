...
const strictInlineTargets = new Set(['preview/index.html', 'preview/app.html', 'preview/print.html']);
const timestamp = new Date().toISOString();
const report = {
  timestamp,
  pages: [],
  summary: { missing_file: 0, missing_ux_css: 0, inline_style_found: 0, strict_inline_style_found: 0 }
};

for (const rel of targets) {
  const abs = path.join(root, rel);
  const row = { file: rel, exists: fs.existsSync(abs), has_ux_css: false, has_inline_style: false, strict_inline_style: strictInlineTargets.has(rel) };
  if (!row.exists) {
    ...
    if (row.has_inline_style && row.strict_inline_style) report.summary.strict_inline_style_found += 1;
  }
...
if (report.summary.missing_file > 0 || report.summary.strict_inline_style_found > 0) {
  process.exit(1);
}
