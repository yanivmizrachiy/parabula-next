...
$pageRows = foreach ($p in $activePages) {
  ...
  if ($hay -match 'גרף עולה|גרף יורד|שיפוע') { $suspected = 'גרף עולה, יורד ושיפוע' }
  elseif ($hay -match 'משוואה ריבועית') { $suspected = 'משוואות ריבועיות' }
...
if ($RunChecks) {
  $checks = @(
    ...
    @{ Name = 'npm-validate-meta'; Command = 'npm run validate:meta' },
